...
# Load data (in a real app, this would be from a database)
@st.cache_data
def load_data():
    # This would be replaced with your actual data loading logic
    # For demo, we'll create sample data similar to what we had
    np.random.seed(42)
    weeks = 35
...
tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "🔍 Optimization", "📈 Analytics", "⚡ Live Dashboard"])
...
with tab1:
    ...
    with col1:
        fig = px.bar(filtered_df, x='SKU', y=['Adjusted_Forecast', 'Current_Stock'], 
                    barmode='group', title='Demand vs Current Stock by SKU')
        st.plotly_chart(fig, use_container_width=True)
    
    with col2:
        region_summary = filtered_df.groupby('Region')[['Adjusted_Forecast', 'Current_Stock']].sum().reset_index()
        fig = px.bar(region_summary, x='Region', y=['Adjusted_Forecast', 'Current_Stock'],
                    barmode='group', title='Demand vs Stock by Region')
        st.plotly_chart(fig, use_container_width=True)

with tab2:
    st.subheader("Production Optimization")
    
    # Simplified optimization function for Streamlit
    ...
    if st.button("🚀 Run Optimization", type="primary"):
        with st.spinner("Optimizing production plan..."):
            ...
            if not results.empty:
                ...
                with col2:
                    fig = px.bar(results, x='SKU', y='Production', color='Plant',
                                title='Optimal Production Allocation')
                    st.plotly_chart(fig, use_container_width=True)
            else:
...
with tab3:
    ...
    with col1:
        ...
        fig = px.line(accuracy_df, x='Week_ID', y=['Forecast_Demand', 'Actual_Demand', 'Adjusted_Forecast'],
...
with tab4:
    st.subheader("Live Supply Chain Dashboard")
    
    # Real-time metrics would go here
    st.info("""
    **Live Monitoring Features** (would be implemented with real data):
    - Real-time inventory tracking
    - Live supplier performance metrics
    - Automated alert system for stock-outs
    - Predictive analytics for demand spikes
    - Integration with ERP systems
    """)
    
    # Placeholder for live data visualization
    st.image("https://via.placeholder.com/800x400?text=Live+Supply+Chain+Dashboard", use_column_width=True)
